...
struct UITestInitialState {
    ...
    private mutating func applySupportRoute(_ screen: UITestScreenRequest) -> Bool {
        ...
        if screen.matches("create", "create-recruitment", "createrecruitment") {
            selectedTab = .home
            let requestedCourseID = screen.identifier ?? MockData.courses.first?.id ?? "course-cheongsong-juwangsan"
        ...
        if screen.matches("host", "host-manage", "hostmanage") {
            selectedTab = .home
            let requestedTripID = screen.identifier ?? MockData.trips.first?.id ?? "trip-cheongsong-juwangsan"
